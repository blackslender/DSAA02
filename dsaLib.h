...
public:
	...
	T& operator[](int index) {
		int i = index; L1Item<T> *p = pHead;
		for (; i > 0 && p != NULL;) {
			i--; p = p->pNext;
		}
		if (i > 0 || p == NULL) throw("Access violation");
		return p->data;
	}
};
/************************************************************************
...
struct AVLNode {
	...
	int Equilibrium() {
		int LHeight = 0;
		int RHeight = 0;
		if (LChild) LHeight = LChild->Height;
		if (RChild) RHeight = RChild->Height;
...
protected:
	...
	AVLNode<T> *RightMost(AVLNode<T> *root) {
		...
		if (root->RChild) return RightMost(root->RChild); else return root;
	}
	//Destroy the node and all of its sub-node
	void Destroy(AVLNode<T>* node) { // Destroy the node but without updating the tree
		if (node == NULL) return;
	...
	T* Find(AVLNode<T> *root, T& key) {
		if (root == NULL) return NULL;
		else if (root->data == key) return &root->data;
		else if (root->data < key) return Find(root->RChild, key);
		else return Find(root->LChild, key);
	}
	...
	AVLNode<T>* FindNode(AVLNode<T> *root, T& key) {
		...
		else if (root->data < key) return FindNode(root->RChild, key);
		else return FindNode(root->LChild, key);
	}
	...
	void Update(AVLNode<T> *node) {
		...
		//Update height
		node->Height = 1;
		if (node->LChild) node->Height = (node->Height > 1 + node->LChild->Height ? node->Height : 1 + node->LChild->Height);
		if (node->RChild) node->Height = (node->Height > 1 + node->RChild->Height ? node->Height : 1 + node->RChild->Height);

		//Update weight
		node->Weight = 1;
		if (node->LChild) node->Weight += node->LChild->Weight;
		if (node->RChild) node->Weight += node->RChild->Weight;

		//Recursive
		if (node->Father) Update(node->Father);
	}
	...
	void RotateLeft(AVLNode<T>* node) {
		...
		//Re-link
		c->Father = a->Father;
		a->RChild = c->LChild; if (a->RChild) a->RChild->Father = a;
		c->LChild = a; a->Father = c;

		//Update type
		c->Type = a->Type;
		a->Type = NLEFT;
		if (a->RChild) a->RChild->Type = NRIGHT;
	...
	void RotateRight(AVLNode<T>* node) {
		...
		//Re-link
		b->Father = a->Father;
		a->LChild = b->RChild; if (a->LChild) a->LChild->Father = a;
		b->RChild = a; a->Father = b;

		//Update type
		b->Type = a->Type;
		a->Type = NRIGHT;
		if (a->LChild) a->LChild->Type = NLEFT;
		if (b->Type == NROOT) pRoot = b;
	...
	void BalanceUp(AVLNode<T> *node) {
		if (node == NULL) return;
		if (!Balance(node)) BalanceUp(node->Father);
	}

	//Insert a new node to the tree at pR root
	void Insert(AVLNode<T>* pR, T& a) {
		if (pR == NULL) { //The root is empty, create new pRoot
		...
		else if (pR->data >= a) Insert(pR->LChild, a);
		else if (pR->data <  a) Insert(pR->RChild, a);
	...
	bool Remove(AVLNode<T>* pR, T& a) {
		...
		if (tmp) { //Found node to delete
			AVLNode<T> *rm = RightMost(tmp->LChild);

			if (rm != NULL) { //If found rightmost node of left subtree, swap its value with current node
				T tmpT = rm->data;
			...
			else  // Left subtree is NULL
			{
				...
				case NRIGHT:
					...
					if (tmp->RChild) tmp->RChild->Father = tmp->Father;
					break;
				default:
					throw("DSA Error");
					break;
	...
	void TraverseLNR(void(*op)(T&), AVLNode<T> *node) {
		if (node == NULL) return;
		if (node->LChild) TraverseLNR(op, node->LChild);
		op(node->data);
		if (node->RChild) TraverseLNR(op, node->RChild);
	};
	//Traverse the tree in NLR order with function void op
	void TraverseNLR(void(*op)(T&), AVLNode<T> *node) {
		if (node == NULL) return;
		op(node->data);
		if (node->LChild) TraverseNLR(op, node->LChild);
		if (node->RChild) TraverseNLR(op, node->RChild);
	};
	//Traverse the tree in LRN order with function void op
	void TraverseLRN(void(*op)(T&), AVLNode<T> *node) {
		if (node == NULL) return;
		if (node->LChild) TraverseLRN(op, node->LChild);
		if (node->RChild) TraverseLRN(op, node->RChild);
		op(node->data);
	};
	...
	void TraverseLNR(void(*op)(T&, void*), void* param, AVLNode<T> *node) {
		if (node == NULL) return;
		if (node->LChild) TraverseLNR(op, param, node->LChild);
		op(node->data,param);
		if (node->RChild) TraverseLNR(op, node->RChild);
	};
	//Traverse the tree in NLR order with function void op
	void TraverseNLR(void(*op)(T&, void*), void* param, AVLNode<T> *node) {
		if (node == NULL) return;
		op(node->data, param);
		if (node->LChild) TraverseNLR(op, param, node->LChild);
		if (node->RChild) TraverseNLR(op, param, node->RChild);
	};
	//Traverse the tree in LRN order with function void op
	void TraverseLRN(void(*op)(T&, void*), void* param, AVLNode<T> *node) {
		if (node == NULL) return;
		if (node->LChild) TraverseLRN(op, param, node->LChild);
		if (node->RChild) TraverseLRN(op, param, node->RChild);
		op(node->data, param);
	};
